 ...
 *       renderer. The message payload should be the target {@link HevcOutputBufferRenderer}, or
 *       null.
 * </ul>
 */
public final class LibHevcVideoRenderer extends BaseRenderer {
   ...
   * HevcOutputBufferRenderer}, or null.
   */
  ...
  public LibHevcVideoRenderer(boolean scaleToFit, long allowedJoiningTimeMs,
    ...
    outputMode = HevcDecoder.OUTPUT_MODE_NONE;
  ...
  public int supportsFormat(Format format) {
    ...
    if (!HevcLibrary.isAvailable() || !MimeTypes.VIDEO_H265.equalsIgnoreCase(format.sampleMimeType)) {
  ...
  public boolean isReady() {
    ...
    if (format != null && (isSourceReady() || outputBuffer != null)
        && (renderedFirstFrame || outputMode == HevcDecoder.OUTPUT_MODE_NONE)) {
   ...
   */
  protected void onQueueInputBuffer(HevcInputBuffer buffer) {
    // Do nothing.
  }
   ...
   */
  protected void skipOutputBuffer(HevcOutputBuffer outputBuffer) {
   ...
   */
  protected void dropOutputBuffer(HevcOutputBuffer outputBuffer) {
   ...
   * for calling {@link HevcOutputBuffer#release()} either immediately or in the future.
   *
   * @param outputBuffer The buffer to render.
   */
  protected void renderOutputBuffer(HevcOutputBuffer outputBuffer) {
    int bufferMode = outputBuffer.mode;
    boolean renderRgb = bufferMode == HevcDecoder.OUTPUT_MODE_RGB && surface != null;
    boolean renderYuv = bufferMode == HevcDecoder.OUTPUT_MODE_YUV && outputBufferRenderer != null;
    lastRenderTimeUs = SystemClock.elapsedRealtime() * 1000;
  ...
  public void handleMessage(int messageType, Object message) throws ExoPlaybackException {
    if (messageType == C.MSG_SET_SURFACE) {
      ...
      setOutput(null, (HevcOutputBufferRenderer) message);
    } else {
  ...
  private void setOutput(
          @Nullable Surface surface, @Nullable HevcOutputBufferRenderer outputBufferRenderer) {
    ...
    if (this.surface != surface || this.outputBufferRenderer != outputBufferRenderer) {
      ...
      outputMode = outputBufferRenderer != null ? HevcDecoder.OUTPUT_MODE_YUV
          : surface != null ? HevcDecoder.OUTPUT_MODE_RGB : HevcDecoder.OUTPUT_MODE_NONE;
      if (outputMode != HevcDecoder.OUTPUT_MODE_NONE) {
        if (decoder != null) {
    ...
    } else if (outputMode != HevcDecoder.OUTPUT_MODE_NONE) {
  ...
  private boolean feedInputBuffer() throws HevcDecoderException, ExoPlaybackException {
    if (decoder == null
  ...
  private boolean drainOutputBuffer(long positionUs, long elapsedRealtimeUs)
      throws ExoPlaybackException, HevcDecoderException {
    if (outputBuffer == null) {
  ...
  private void renderRgbFrame(HevcOutputBuffer outputBuffer, boolean scale) {
    if (bitmap == null
