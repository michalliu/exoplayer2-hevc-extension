 ...
 */
/* package */ class HevcRenderer implements GLSurfaceView.Renderer {
  ...
  private final AtomicReference<HevcOutputBuffer> pendingOutputBufferReference;
  ...
  private HevcOutputBuffer renderedOutputBuffer; // Accessed only from the GL thread.
   ...
   */
  public void setFrame(HevcOutputBuffer outputBuffer) {
    HevcOutputBuffer oldPendingOutputBuffer = pendingOutputBufferReference.getAndSet(outputBuffer);
  ...
  public void onDrawFrame(GL10 unused) {
    HevcOutputBuffer pendingOutputBuffer = pendingOutputBufferReference.getAndSet(null);
    ...
    HevcOutputBuffer outputBuffer = renderedOutputBuffer;
    // Set color matrix. Assume BT709 if the color space is unknown.
    float[] colorConversion = kColorConversion709;
    switch (outputBuffer.colorspace) {
      case HevcOutputBuffer.COLORSPACE_BT601:
        colorConversion = kColorConversion601;
        break;
      case HevcOutputBuffer.COLORSPACE_BT2020:
        colorConversion = kColorConversion2020;
        break;
      case HevcOutputBuffer.COLORSPACE_BT709:
      default:
        break; // Do nothing
    }
